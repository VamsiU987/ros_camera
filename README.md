...
4. **Download ROS 2 Jazzy (Windows binary ZIP)**  
   Download from:
   ```
...
Install ROS 2 Jazzy using Homebrew:
```bash
brew update
...
> Ensure the paths match your local setup if you did not use the recommended directories.
...
If the webcam does not initialize correctly, run the following command **once**:
```bash
